...
2) Open the editor preferences settings for the plugin, which is avaiLable in Preferences->EOS Plugin. All EAC options are located under the Tools subheader.
...
5) (Recommended) Set `Path to EAC splash image` to a 800x450 PNG that will display in the EAC boot-strapper.
6) (Optional)
