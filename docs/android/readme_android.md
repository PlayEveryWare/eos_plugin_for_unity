...
* The Android Build Support <a href="https://docs.unity3d.com/hub/manual/AddModules.html">module</a>.
    > [!INFO]
    > You can <a href="https://www.openlogic.com/openjdk-downloads">install OpenJDK</a> externally so it is easier to set environment variables.
    
    > [!INFO]
    > `Android Studio` or other official sites can be used to get the `SDK`,`NDK`, or `Gradle` as needed.
    
    > [!INFO]
    > Remember have unity point to these changes `Edit -> Preferences -> External Tools`.
...
> [!WARNING]
> The EOS Overlay is not implemented yet.   
